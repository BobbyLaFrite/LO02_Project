...
public class PlayerGroup {
	...
	//private List<Player> exceptionsAccusationList; ##Pas utile je pense car on va pas actualiser la liste à chaque fois donc pas d'attributs##
	...
	public List<Player> getRevealedPlayer(){
		...
		while (playerIT.hasNext()) {
			...
			if (actuPlayer.getRole().getIsRevealed()){//Si le role est révélé
	...
	//public int getUnrevealedPlayerNumber() { Pas utile je pense car on va pas actualiser la liste à chaque fois donc pas d'attributs##
	...
	public List<Player> getTarget(String typeOfTarget,Player exception) {
		...
		if (typeOfTarget.contains("accusation")) { //##Modif car il peut y avoir plusieurs types d'exeption en même temps
			targets.removeAll(this.getRevealedPlayer());
		}
		if (typeOfTarget.contains("alive")) { //##Modif car il peut y avoir plusieurs types d'exeption en même temps
