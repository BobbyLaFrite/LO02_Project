...
//	private Player prevPlayer = null;
	...
	public void playRound() {
		...
		while (playerIt.hasNext()) {
			Player cPlayer = playerIt.next();
			if (cPlayer.getStrategie().chooseRole().equalsIgnoreCase("Villager")) {
				cPlayer.getRole().setRole("Villager");
			}
			else {
				cPlayer.getRole().setRole("Witch");
			}
		}
		...
		while (!this.shouldRoundEnd()) {
			...
			System.out.println("\n-------------------------------------\nC'est a "+playingPlayer.getName()+" de jouer !");
			if (!playingPlayer.getStrategieAsString().equalsIgnoreCase("HumanPlayer")) {//Si le joueur est une IA, on attend un peu
				try {
					Thread.sleep(2000);
				} catch (InterruptedException e) {
					System.out.println("L'attente entre les IA a ete interrompue.");
				}
			}
			nextPlayer=this.playingPlayer.play(nextPlayer.getIsAccused());
		...
		it = this.playerGroup.getIterator();
		while (it.hasNext()) {
			roundWinner=it.next();
			System.out.println(roundWinner.getName()+" possede "+String.valueOf(roundWinner.getScore())+" points.");
		}
